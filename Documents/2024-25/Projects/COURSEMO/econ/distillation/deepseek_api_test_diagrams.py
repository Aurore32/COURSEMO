...
df = pd.read_csv('./econ/distillation/al-econ-diagrams-dataset.csv')

questions = df['Prompt'].tolist()[268:]
for i in range(len(questions)):
    question = questions[i]
    try:
...
- Preserve all labels verbatim (e.g. description mentions "quantity demanded shifted from Q1 to Q2" -> properly label this on diagram)
        ...
        old_df = pd.read_csv('./econ/distillation/al_econ_diagrams_deepseek_r1.csv')
        new_df = pd.DataFrame([])
