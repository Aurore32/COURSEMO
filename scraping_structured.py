...
# 下列操作可以看一下scraping_mcq.py

subject_name = 'IGCSE PHYSICS'
...
def getAnswers(url):
    ...
    questions = soup.find_all("div", class_="p-ms")
    texts = []
...
# 这个是结构题专用的一个把题目分成小题的代码 写的非常的烂 如果有人能重新写一个的话诚恳建议快点去写

def split(soup: BeautifulSoup):
...
# 用上面的split把每个提取出来的大题分成小题

def stripQuestions(questions: list):
    final_questions = []
    full_indices = []
    for i in range(len(questions)):
        question_list = split(questions[i])
...
# 用上面的split把每个提取出来的mark scheme分成小题
...
for i in range(100): # 1-100页的题目
    ...
    for question in question_list:
        if question[1] in answer_indices:
            question.insert(1, answer_list[answer_indices.index(question[1])][0])
            final_question_list.append(question)
    

    df = pd.DataFrame(final_question_list, columns=['question',  'answer', 'index'])
    df.to_csv('./physics/distillation/igcse_physics_structured_dataset.csv')
